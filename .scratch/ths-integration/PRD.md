...
- [x] K 线源（THSKlineSource，1d + 周/月重采样，commit 3f227d8）
- [x] 财务指标 enrichment（监控股票 ROE/毛利率/净利率/负债率入因子库，commit 6588bbd）
- [x] 异动归因（官方当日异动原因入库 + 简报集成，commit 7ffc199）
- [x] 事件日历增强（公司行为并入，commit d889fde）
- [ ] 行情快照 prices/snapshot 接入（可选：替代/校验东财延时）
- [ ] 同花顺指数/成分股（a-share-index）接入（基准用）

## 限流情况

文档未公开限流；实测一天内估值批量 ~56 次 + 财务指标 ~20 次 + 异动 1 次 + 公司行为 25 次后
仍全部正常返回 → 免费档额度对个人使用足够，但无量化上限数据。已采用保守设计：
批量接口（≤100/次）、按需拉取（指标/公司行为仅监控股票）、缓存（报告期未变不重拉）。
